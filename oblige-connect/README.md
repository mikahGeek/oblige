...
> **Already know this sample? Run: `make hurry.first` - This command will install app deps, build, and deploy your Serverless application using SAM guided deploy. Alternatively, run `make hurry` if your sam config is already set up.**

Build and deploy your application for the first time by running the following commands in your shell:

```bash
oblige-connect$ make build
oblige-connect$ make deploy.guided
```
...
```bash
oblige-connect$ make run
oblige-connect$ curl http://localhost:3000/hello
```
...
```bash
oblige-connect$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
oblige-connect$ make dev
oblige-connect$ make test
```
...
```bash
oblige-connect$ aws cloudformation delete-stack --stack-name oblige-connect
```
...
```bash
oblige-connect$ pipenv lock -r > hello_world/requirements.txt
```
