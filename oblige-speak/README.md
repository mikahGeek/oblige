...
```bash
oblige-speak$ make build
oblige-speak$ make deploy.guided
```
...
```bash
oblige-speak$ make run
oblige-speak$ curl http://localhost:3000/hello
```
...
```bash
oblige-speak$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
oblige-speak$ make dev
oblige-speak$ make test
```
...
```bash
oblige-speak$ aws cloudformation delete-stack --stack-name oblige-speak
```
...
```bash
oblige-speak$ pipenv lock -r > hello_world/requirements.txt
```
